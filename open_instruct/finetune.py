...
class FlatArguments:
    ...
    logging_steps: Optional[int] = field(
        default=5, metadata={"help": "Log the training loss and learning rate every logging_steps steps."}
    )
    ...
    keep_last_n_checkpoints: int = field(
        default=-1, metadata={"help": "How many checkpoints to keep in the output directory. -1 for all."}
    )
    ...
    validation_steps: Optional[int] = field(
        default=0,
        metadata={"help": "Run validation every X steps (using test_dataloader). Set to None to disable."}
    )
    ...
    wandb_project_name: str = "Tulu3"
    """The wandb's project name"""
    wandb_entity: Optional[str] = None
    """The entity (team) of wandb's project"""
    push_to_hub: bool = False
    """Whether to upload the saved model to huggingface"""
    hf_entity: Optional[str] = "HannanM"
...
def evaluate_validation_loss(model, dataloader, accelerator, embedding_size, args):
    model.eval()
    total_loss = 0.0
    total_tokens = 0
    with torch.no_grad():
...
def main(args: FlatArguments, tc: TokenizerConfig):
    ...
    test_dataloader = DataLoader(
        test_dataset, shuffle=False, collate_fn=collate_fn, batch_size=args.per_device_train_batch_size
